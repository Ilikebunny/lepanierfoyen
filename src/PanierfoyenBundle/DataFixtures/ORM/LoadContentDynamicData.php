...
class LoadContentDynamicData extends AbstractFixture implements OrderedFixtureInterface, ContainerAwareInterface {
    ...
    public function load(ObjectManager $manager) {
        ...
        $fileContent = $import->CSV_to_array('contenue.csv');

        foreach ($fileContent as $numRow => $row) {
            if ($numRow != 1) {
                $entity = new ContentDynamic();
